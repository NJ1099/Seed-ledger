...
### 📌 2026-09-24 배포 후 현황 (커밋 `d3faccb` · 배포 확인됨)

배포 판정은 `/api/config-status` 에 **새 코드에만 있는 `NAVER_CLIENT_ID` 필드**가 나타나는
것으로 했다(200 도 갱신 시각도 증거가 아니다).

| 항목 | 상태 |
|---|---|
| 뉴스 cron 시크릿 | ✅ **해결.** Render `secretRequired:true` · 키 없이·틀린 키로 호출 모두 **401**(503 아님 = 시크릿 존재 + fail-closed 작동) |
| KRX | 🔧 **`CD010` → `CD006` 으로 전진.** 비밀번호 만료는 풀렸고 이제 `KRX_PW` 값 갱신만 남았다 |
| 네이버 | 🔧 **원인 확정: Render 에 키가 없다.** `NAVER_CLIENT_ID:false`·`NAVER_CLIENT_SECRET:false` |

**🔧 남은 사용자 액션 2건 (둘 다 Render 환경변수)**

1. **`NAVER_CLIENT_ID` · `NAVER_CLIENT_SECRET` 등록** — 값은 로컬 `.env` 에 있고
   **구 주소(`openapi.naver.com`)로 정상 동작하는 것을 실측**했다(2026-09-24). 넣으면
   뉴스가 Google News RSS 폴백에서 네이버 OpenAPI 로 바뀐다.
2. **`KRX_PW` 를 새 비밀번호로 갱신** (Render + 로컬 `.env` 양쪽).
   - ⚠️ **네이버 소셜 로그인으로는 안 풀렸다** — `KRX_ID`/`KRX_PW` 직접 로그인 계정의
     만료 플래그는 그대로였다(네이버로 로그인한 뒤에도 `CD010` 유지).
   - ✅ **사용자가 비밀번호를 바꾼 뒤 `CD010` → `CD006` 으로 전이했다**(2026-09-24).
     만료는 풀렸다. 그런데 **옛 비밀번호가 그대로 있는 로컬에서도 같은 `CD006`** 이라,
     지금은 **값이 안 맞는 상태**로 보인다 → 새 값을 두 곳에 넣어야 한다.
   - KRX 는 `_error_code` 만 주고 메시지를 주지 않아 단정은 못 한다. 근거는
     **관측된 전이**(CD010 → 비번 변경 → CD006)와 **로컬 동일 재현**이다.
   - 🔴 **로그인 시도를 반복하지 말 것** — 코드 주석대로 KRX 가 봇으로 판단하면
     **계정이 잠긴다.** `krx-auth-check` 강제 판정은 꼭 필요할 때만.

✅ **시크릿 일치 검증 완료** (2026-09-24 17:43 KST). 올바른 키로 `?scheduled=1` 을 불러
`HTTP 200 {"ok":true,"skipped":"outside-window"}` 를 받았다 — **인증은 통과했고(불일치면 401)
창 밖이라 발송은 되지 않았다.** GitHub Secret = Render = 로컬 `.env` 가 같은 값이다.

> 🔴 **이 검증을 다시 할 때의 규칙**: 슬롯(`NEWS_PUSH_SLOTS_KST` = 09:30·18:00)과
> `NEWS_TRIGGER_GRACE_MIN`(420) 으로 **창 밖임을 먼저 계산하고**, 다음 슬롯까지 10분 이상
> 남았을 때만 부를 것. 창 안에서 부르면 **진짜로 발송된다.**
> ⚠️ 시각은 이 PC 가 아니라 **서버의 `/api/config-status` 의 `ts`** 를 쓸 것 — Git Bash 에서는
> `TZ=Asia/Seoul` 이 먹지 않아 UTC 가 나온다(9시간 어긋난다).
> ⚠️ `NEWS_TRIGGER_GRACE_MIN` 을 소스에서 정규식으로 긁으면 `Math.max(5, …)` 의 **5** 가
> 잡힌다. 실제 기본값은 **420** 이다.

### ⚠️ Actions cron 지연이 grace 한계에 13분까지 다가와 있다 (2026-09-24 실측)

최근 10회 실측: **최소 239분 · 최대 407분 · 평균 293분**. `grace` 는 **420분**이다.

| 크론(UTC) | 슬롯(KST) | 실제 실행(KST) |
|---|---|---|
| `30 0 * * *` | 09:30 | 대개 14:00~14:20 |
| `0 9 * * *` | 18:00 | 대개 22:00~23:10 (한 번 **00:47**, 지연 407분) |

🔴 **지연이 420분을 넘기면 서버가 `{"ok":true,"skipped":"outside-window"}` 로 200 을 준다** —
워크플로는 **초록불인데 뉴스는 안 나간다.** 200 으로 답하는 건 `curl -fsS` 가 4xx 에서
스텝을 실패시키기 때문에 일부러 그런 것인데, 그 대가로 **진짜 누락이 안 보인다.**
(budongsan 라운드 39 의 "초록불은 일을 했다는 뜻이 아니다"와 같은 계열이다.)

확인 결과 **아직 실제로 누락된 회차는 없다** — 407분 회차도 로그가
`{"ok":true,"slot":"2026-09-21:18:00"}` 로 발송됐다. 13분 차이였다.

⬜ **후보 대응**(사용자 판단 필요 · 발송 시각 동작이 바뀐다):
① `NEWS_TRIGGER_GRACE_MIN` 을 600 정도로 올린다(환경변수라 재배포 불필요) ·
② 워크플로가 응답 본문의 `skipped` 를 보고 `::warning::` 를 띄운다(budongsan
`scripts/lib/annotate.mjs` 방식) — **초록불인데 누락된 것을 사람이 알 수 있게.**

> 🔴 **발송 시각 직전에는 배포 repo 에 푸시하지 말 것.** Render 재배포는 콜드스타트를
> 동반해서, 그 순간 도착한 워크플로 요청이 실패할 수 있다.
...
사용자 요청(텔레그램): "부동산 메인과 시드렛저, ai오피스 이 3가지의 모든 코딩을 분석하고
...
`syncPullNow()` 가 안에서 `restoreFromSyncPayload(j.payload)` 까지 해 버린다. 그런데
`tryFirstSyncAction` 은 그 **뒤에** 로컬 건수를 세고 물어봤다:

```js
const result = await syncPullNow();        // ← 이 안에서 localStorage 가 이미 갈렸다
const localCount = (() => { lsGetAccounts() … })();   // ← 덮어써진 뒤에 센다
const ok = localCount === 0 || confirm(`… 로컬 데이터: ${localCount}건
                                        취소하시면 로컬 데이터를 유지하고 …`);
if (ok) { … } else { await syncPushNow(); status = '✓ 로컬 데이터를 새로 백업했습니다'; }
```

- 프롬프트의 "로컬 데이터: N건"이 실은 **원격 백업의 건수**다 — 사용자는 자기 것이 아닌
  숫자를 보고 판단한다.
- **"취소"를 누르면 그 덮어써진 데이터를 다시 백업**하며 "✓ 로컬 데이터를 텔레그램에 새로
  백업했습니다"라고 표시한다. **원래 장부는 사라졌고 복구 수단이 없다.**
- `ok` 분기의 주석("restoreFromSyncPayload 는 syncPullNow 안에서 이미 적용됨")은 작성자가
  그 사실을 **알고 있었음**을 보여 준다 — `else` 분기만 그대로 남았다.

→ `syncPullNow({ apply: false })` 로 **받아만 오고**, 로컬 건수를 **pull 전에** 세고,
확인을 받은 뒤에만 `restoreFromSyncPayload(result.payload)` 를 부른다.
⚠️ **`apply` 기본값은 `true` 로 두었다** — 수동 「복원」 버튼은 인자 없이 부르므로
기본값을 false 로 바꾸면 그 버튼이 조용히 아무 일도 하지 않게 된다(테스트가 검사한다).

### 2. 🔴 [높음] 연동 해제가 서버의 502(폐기 실패)를 통째로 버렸다

```js
try { await fetch('/api/sync/disconnect', …); } catch {}   // 응답을 안 본다
syncWriteCred(null);                                       // 무조건 "해제됨"
```

서버는 폐기 기록에 실패하면 **일부러 502** 를 주고 주석까지 달아 뒀다 — *"폐기를 기록하지
못했는데 성공이라고 답하면, 사용자는 끊었다고 믿는데 cred 는 살아 있다."* 클라이언트가
그걸 버려서 **라운드 32 가 만든 폐기 기능이 실사용 경로에서 무력화**돼 있었다. Upstash
일시 장애 중 해제를 누르면 유출된 cred 가 `SYNC_CRED_TTL_DAYS`(기본 180일)까지 유효하다.

→ 응답을 확인하고 `{ revoked, error }` 를 돌려주며, 실패하면 호출부가 사용자에게 알린다
(로컬 자격은 어느 쪽이든 지운다 — 이 기기에서 못 쓰게 하는 것은 확실히 해 둔다).

### 3. 🔴 [높음] 속도 제한이 6곳 빠져 있었다 — 그리고 **주석이 반대로 단언하고 있었다**

`stock-detail`(라운드 34)의 주석이 *"속도 제한이 필요하다 — 이 파일의 다른 외부 호출
엔드포인트에는 **다 붙어 있다**"* 라고 적혀 있었는데 **사실이 아니었다.** 실제로는 그 한
곳만 고쳐졌고 `quotes`·`history`·`stock-search`·`stock-news`·`pension-flows`·
`nps-portfolio` 는 전부 빠져 있었다. **그 틀린 문장이 다음 점검을 통과시키는 근거가 됐다.**

- `/api/quotes` — 티커 200개 × 소스별 1~2회 = **요청 하나가 아웃바운드 최대 ~400건.**
  티커 수 상한만으로는 반복 호출을 못 막는다.
- `__search:{q}`·`__news:{q}` — **검색어를 매번 다르게 보내면 캐시를 통째로 우회한다**
  (`__detail:{code}` 와 같은 구조). 게다가 신선한 키를 대량으로 밀어넣으면
  `pruneQuoteCache`(오래된 것부터 삭제)가 `__indices`·`__movers_kr` 를 밀어내
  **아웃바운드가 오히려 더 늘어난다.**
- `/api/history` — 캐시가 아예 없어 **매 호출이 곧 야후·업비트 호출**이다.

한도는 **정상 사용의 15배 여유**로 잡았다(화면은 15초마다 quotes 1회 = 4회/분 → 한도 60).
→ **주석의 단언을 테스트로 옮겼다** — 핸들러 목록을 소스에서 읽어 `rateLimited(` 가
있는지 검사한다. 이제 빠뜨리면 테스트가 이름을 대며 실패한다.

### 4. 🔴 [높음] `pension-flows` 의 `source` 가 미검증 — DART 일일 1만 호출 한도를 태울 수 있었다

`daysBack` 은 1~180 으로 조이는데 `source` 는 화이트리스트가 없었다. 임의 문자열이면
`sourceParam !== 'goinsider'` 와 `sourceParam !== 'dart'` 가 **둘 다 참**이라
**DART 파이프라인과 goinsider 폴백이 매번 함께 돈다.** 그 값이 캐시 키에도 들어가서
`?source=a1`, `?source=a2` … 로 **24시간 캐시를 100% 우회**할 수 있었다. 한도를 넘기면
`DART_STATUS_MSG['020']` 상태가 되어 **연기금 기능이 자정까지 죽는다.**
바로 옆 `handleKrxPensionTrading`·`handleKrxInvestorFlows` 는 이미 화이트리스트를 쓴다 —
패턴을 이 한 곳만 안 따랐다. `nps-portfolio` 의 `uddi`(URL 경로에 그대로 들어간다)·`page` 도 같은 구조였다.

### 5. `tgEscapeHtml` 이 `"` 를 안 막았다 — 뉴스 한 건이 그 회차 발송 전체를 죽인다

주석("텔레그램은 `&` `<` `>` 만 escape 하면 됨")은 **텍스트 노드에만** 맞다. 결과가
`<a href="...">` **속성값**으로도 들어가는데 `safeNewsUrl` 은 `^https?://` 만 보고 `"` 를
걸러내지 않는다. 인코딩되지 않은 `"` 가 하나 섞이면 태그가 깨져 `sendMessage` 가
`can't parse entities` 로 실패하고 **일일 뉴스가 로그에만 남고 안 나간다**(같은 뉴스 목록이
캐시 TTL 동안 유지되므로 재시도도 같은 이유로 실패).

### 6. `handleHistory` 의 500 이 내부 에러 메시지를 노출했다

전역 catch 는 *"err.message 를 그대로 내보내면 내부 경로가 노출된다"* 며 상세를 로그로만
보내는데, 이 핸들러만 그 규율을 어겼다. `logLine('error','history.fail',…)` + `internal error`.

### 7. 정리 — `q1fy27.html` 삭제

**0바이트 · 어디서도 참조 안 됨 · `PUBLIC_FILES` 화이트리스트에도 없음 · 배포 클론에도 없음.**
2026-07-20 에 생긴 빈 파일이었다.

확인했는데 **문제 없던 것**: 경로 순회·정적 서빙 3중 방어 · `app.js` 의 `innerHTML` 99곳
전수(외부 데이터는 모두 `esc()` 통과) · cred 발급/검증/만료/폐기 설계 · `clientIp()` 외
`x-forwarded-for` 직접 참조 0건 · 유예 창·자정 경계 · `npm test` 실발송 가드 ·
미사용 npm 의존성 없음 · `sw.js::SHELL_FILES` 와 실제 파일 일치.

### ⬜ 남은 것 (이번에 안 고침)

- **자동 백업 실패가 `console.warn` 으로만 끝난다**(`schedulePushDebounced`) — 사용자는
  계속 입력하면서 백업이 되고 있다고 믿는다. 라운드 34 가 추가한 `j.pinned`/`j.pinWarning`
  도 클라이언트가 **한 번도 읽지 않는다**(핀 실패 시 복원하면 옛 백업이 돌아온다).
- **뉴스 중복 발송 방지 키가 프로세스 메모리(`lastNewsPushSlot`)에만 있다** — Render Free
  슬립/재시작이면 같은 slot 이 두 번 나갈 수 있다. Upstash(`seed:notices` 등)가 이미
  붙어 있으니 키만 옮기면 된다.
- **`quote-cache.json` 이 읽고-고쳐-쓰기** — `writeJSON` 은 tmp+rename 으로 파일 단위
  원자성만 보장한다. 자동 폴러와 조회가 겹치면 한쪽 캐시 기록이 조용히 사라진다.
- **해외 거래소 코인 손익에 김치프리미엄이 섞인다** — 평가금액은 업비트 KRW(김프 포함),
  원가는 USD 평단 × 환율(김프 없음). 아무것도 안 했는데 +2~5% 수익이 떠 있다. 반대 방향
  (업비트 미상장 → 글로벌 폴백)도 있다.
- **백업 한도 경고가 UTF-16 길이를 바이트로 센다** — 한글은 3배라, 경고가 안 떴는데
  POST 가 4MB 를 넘어 413 으로 실패할 수 있다.
- **`pickCashKRW` 가 이름 정규식에 걸린 필드 중 최댓값을 예수금으로 채택한다** — 평가액이
  포함된 필드가 걸리면 총자산이 부풀려진다(라운드 34 ①과 같은 계열).
- **`parseKRNumber`(실패 시 NaN) vs `parseKrNumber`(실패 시 null)** — 이름이 한 글자 차이고
  실패값이 다르다. 섞으면 `NaN != null` 이 참이라 null 검사를 전부 통과한다.
- SRI·CSP·보안 헤더가 없다(localStorage 에 전 재산 장부와 sync cred 가 있다).
- 주석 드리프트 3곳(`pdfImport` 의존성 · `httpsPostForm` · `httpsGetRaw` 설명).
