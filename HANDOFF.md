...
최종 업데이트: 2026-06-10 (라운드 20 — 사이드카를 네이버 속보 기반 실시간 발동/해제 판정으로 전환 + 공지 2개 이상 시 view all)
...
**알려진 제약 / 후속**:
- 뉴스 지연(평균 1~5분)으로 발동 직후 수 분은 못 잡을 수 있음. 5분 윈도우 + 60초 폴링 + 메모리 래치로 완화.
- `fetchNaverOpenApiNews` 가 HTTP 실패 시 `[]` 반환(키 미설정 `null` 과만 구분) → 실패를 normal 로 오인 가능. 단 평시 "사이드카" 쿼리는 항상 다수 기사 반환(available=true)이라 Render 에선 사실상 발동 기사 유무로만 갈림. 키 미설정 시에만 unknown.
- 더 정밀하려면 KRX 멤버십/증권사 API 의 구조화 피드 필요(라운드 19 메모 참조).
